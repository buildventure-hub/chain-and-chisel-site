// Chain & Chisel — Combined Email + CRM Worker
// Handles: Resend emails (notification + auto-reply) + Airtable CRM record creation
// Replaces: EmailJS frontend calls
...
export async function onRequestPost(context) {
  const { RESEND_API_KEY, AIRTABLE_TOKEN, AIRTABLE_BASE_ID } = context.env;
  ...
  try {
    ...
    if (res.ok) {
      results.email_notify = true;
    } else {
      const err = await res.text();
      errors.push(`notify: ${err}`);
      console.error("[send.js] Notification email failed:", err);
    }
  } catch (e) {
    errors.push(`notify: ${e.message}`);
    console.error("[send.js] Notification email exception:", e);
  }
  ...
  try {
    ...
    if (res.ok) {
      results.email_reply = true;
    } else {
      const err = await res.text();
      errors.push(`reply: ${err}`);
      console.error("[send.js] Auto-reply email failed:", err);
    }
  } catch (e) {
    errors.push(`reply: ${e.message}`);
    console.error("[send.js] Auto-reply email exception:", e);
  }

  // ── 3. Airtable CRM record ──────────────────────────────────────────────────
  try {
    const address = [street, city, zip].filter(Boolean).join(", ");
    const res = await fetch(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent("Commissions")}`,
      {
        method: "POST",
        headers: {
    ...
    if (res.ok) {
      ...
      const err = await res.text();
      errors.push(`crm: ${err}`);
      console.error("[send.js] Airtable write failed:", err);
    }
  } catch (e) {
    errors.push(`crm: ${e.message}`);
    console.error("[send.js] Airtable exception:", e);
  }

  // ── Response ────────────────────────────────────────────────────────────────
  // Emails are the critical path — succeed if at least one email went through
  const emailOk = results.email_notify || results.email_reply;
  return json(
    { ok: emailOk, results, errors: errors.length ? errors : undefined },
    emailOk ? 200 : 500
  );
}
...
function buildNotificationEmail({ from_name, from_email, phone, street, city, zip, species, circumference, condition, message }) {
  ...
  return `
<div style="font-family:system-ui,Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f0f10;border:1px solid rgba(255,255,255,0.12);border-radius:8px;overflow:hidden;">
...
function buildAutoReplyEmail({ from_name, from_email, phone, species, circumference, condition, message }) {
  return `
<div style="margin:0;padding:0;background:#0f0f10;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
